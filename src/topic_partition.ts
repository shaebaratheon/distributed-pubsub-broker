...
export interface TopicPartitionContext {
...
export class TopicPartitionEngine {
  private registry = new Map<string, any>();
  private processed = 0;
  private errors = 0;

  constructor(private readonly config: TopicPartitionConfig = { timeoutMs: 5000, maxRetries: 3, enabled: true }) {}
  ...
  public async process(ctx: TopicPartitionContext): Promise<{ digest: string; durationMs: number }> {
