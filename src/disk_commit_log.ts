...
export interface DiskCommitLogConfig {
  timeoutMs: number;
  maxRetries: number;
  enabled: boolean;
}

export interface DiskCommitLogContext {
...
export class DiskCommitLogEngine {
  private registry = new Map<string, any>();
  private processed = 0;
  private errors = 0;

  constructor(private readonly config: DiskCommitLogConfig = { timeoutMs: 5000, maxRetries: 3, enabled: true }) {}
  ...
  public async process(ctx: DiskCommitLogContext): Promise<{ digest: string; durationMs: number }> {
