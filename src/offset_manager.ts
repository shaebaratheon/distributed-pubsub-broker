...
export interface OffsetManagerConfig {
  timeoutMs: number;
  maxRetries: number;
  enabled: boolean;
}

export interface OffsetManagerContext {
...
export class OffsetManagerEngine {
  private registry = new Map<string, any>();
  private processed = 0;
  private errors = 0;

  constructor(private readonly config: OffsetManagerConfig = { timeoutMs: 5000, maxRetries: 3, enabled: true }) {}
  ...
  public async process(ctx: OffsetManagerContext): Promise<{ digest: string; durationMs: number }> {
